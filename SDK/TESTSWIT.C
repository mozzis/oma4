...
{
  FLOAT tracks, flength, testscanslo, testscanshi;
  USHORT port = 0x300;
  ...
  printf("Enter number of scans for low signal: ");
  scanf(" %f ", &testscanslo);

  printf("Enter number of scans for high signal: ");
  scanf(" %f ", &testscanshi);

  printf("Scans lo: %f  high: %f\n",testscanslo, testscanshi);

  SetParam(DC_DAPROG, 6);
  GetOMAString(DS_DANAME, 6, DaName);
  printf("Data Acquisition Mode is %s\n",DaName);

  SetParam(DC_I, testscanslo);
  ...
  print_data(0, 24, 24, "Start & End of first track:");

  interval();

  ReadCurveFromMem(Data, length, 1);

  print_data(0, 24, 24, "Start & End of second track:");

  interval();

  SetParam(DC_I, testscanshi);
